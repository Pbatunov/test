...
module.exports = {
    create: function(configLocal) {
        const config = _.merge({}, configMain, configLocal);

        if (config.baseTasks) {
            for (const taskName in config.baseTasks) {
                const taskConfig = config.baseTasks[taskName];
        ...
        if (config.otherTasks) {
            for (const seriaName in config.otherTasks) {
                const seria = config.otherTasks[seriaName];

                for (const taskName in seria) {
                    const taskConfig = seria[taskName];
